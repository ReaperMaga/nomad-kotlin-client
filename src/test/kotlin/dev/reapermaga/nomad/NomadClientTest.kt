...
class NomadClientTest : StringSpec({
    ...
    "update job" {
        ...
        val job = client.jobs.read(createdJob)
        job.shouldNotBeNull()
        job.datacenters.size shouldBe beGreaterThanOrEqualTo(0)
        client.jobs.update(createdJob) {
            id = createdJob
            datacenters = listOf("dc1", "dc2")
            type = "batch"
            group {
                name = "example-group"
                network {
    ...
    "list node allocations" {
        ...
        if (attempts > 0) {
            val allocationId = allocations.first().id
            val nodeId = allocations.first().nodeId
            val allocation = client.nodes.listAllocations(nodeId).find { it.id == allocationId }
            allocation.shouldNotBeNull()
            println("Allocation: $allocation")
        } else {
            println("Allocation did not reach running state in time, skipping read allocation test.")
